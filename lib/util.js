...
const errorStrings = {
  ...
  diff: 'Showing differences between two values:',
...
function stackError(start, msg) {
  return capture(new Error(msg), start);
}
...
function display(value, colors, limit, options, fullTrace) {
  if (colors == null)
    colors = false;
...
function show(error, colors, fullTrace) {
  ...
  if (isDisplayable(error)) {
    const value = get(error, 'value');
    const limit = get(error, 'limit');
    const options = get(error, 'options');
    const text = display(value, colors, limit, options, fullTrace);
