...
function show(error, colors) {
  const out = [];

  if (isDisplayable(error)) {
    const {value} = error;
    const text = display(value, colors);
...
// Leave a token for ourselves so we can
// find the end of the bmocha code (and
// the start of the real tests). This
// hack works because we're the last
// required file.
