...
class Mocha extends Runnable {
  ...
  set(options) {
    ...
    if (typeof options.getLine === 'function')
      this.getLine = options.getLine;
  ...
  async run(funcs) {
    ...
    this.report.getLine = this.getLine;
...
class Base {
  ...
  error(error, depth) {
    ...
    if (diff.isShowable(error, this)) {
      const text = diff.show(error, this);
  ...
  epilogue(mocha) {
    ...
    for (let i = 0; i < mocha.errors.length; i++) {
      const id = mocha.results.length + i + 1;
...
function toMessage(error) {
  ...
  if (error.generatedMessage) {
    const operator = diff.getOperator(error);
...
function toStack(error) {
  ...
  if (typeof __filename === 'string'
      && __filename.length > 13) {
    // Exclude bmocha/bin/ and bmocha/lib/
    // from the stack trace. Note that this
    // is probably very v8 specific.
    const dirname = __filename.slice(0, -13);
    const sep = dirname[dirname.length - 1];
    const bin = stack.indexOf(` (${dirname}bin${sep}`);
    const lib = stack.indexOf(` (${dirname}lib${sep}`);
