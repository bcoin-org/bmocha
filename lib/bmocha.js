...
class Mocha extends Runnable {
  ...
  _init() {
    for (const method of [this.describe, this.it]) {
      method.only = (name, func) => method(name, func, 'only');
      method.skip = (name, func) => method(name, func, 'skip');
    }
  }
  ...
  _describe(name, func, action) {
    return this._suite().describe(name, func, action);
  }

  _it(name, func, action) {
    return this._suite().it(name, func, action);
  }
  ...
  async run(funcs) {
    ...
    // If the reporter needs the total
    // amount of tests up front, we
    // pre-initialize all suites.
    // Otherwise, we can start executing
...
class Suite extends Runnable {
  ...
  beforeEach(name, func) {
    ...
    this.beforeEaches.push(hook.init());
  }
  ...
  afterEach(name, func) {
    ...
    this.afterEaches.push(hook.init());
  }

  describe(name, func, action) {
    if (name === '')
  ...
  static from(mocha, func) {
    ...
    return new Suite(mocha, '', func).init();
  }
}
  ...
  async exec() {
    return new Promise((resolve, reject) => {
      ...
      if (isCallbackable(this.func)) {
        ...
        try {
          result = this.func.call(ctx, done);
        } catch (e) {
          job.reject(e);
          return;
        }
        ...
        try {
          if (this.func.length > 0)
            result = this.func.call(ctx, ctx);
          else
            result = this.func.call(ctx);
        } catch (e) {
          job.reject(e);
          return;
        }
...
class Reporter {
  ...
  epilogue(mocha) {
    ...
    for (const test of mocha.results) {
      ...
      const stack = color('stack', sanitize(test.stack));
...
class ListReporter extends Reporter {
  ...
  testEnd(test) {
    ...
    if (test.failed) {
      ...
      this.write('  '
        + color('fail', `${test.id}) ${test.fullName()}`)
        + '\n');
...
function snapshot(target, values) {
...
function restore(target, backup) {
  assert(target && typeof target === 'object');
  assert(Array.isArray(backup));

  for (const [key, desc] of backup) {
    if (!desc) {
      delete target[key];
      continue;
    }
