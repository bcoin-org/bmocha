...
class Mocha extends Runnable {
  ...
  async run(funcs) {
    ...
    try {
      this.suite.init(funcs, this.run);
    } catch (e) {
...
class Context {
  ...
  bail(enabled) {
    if (arguments.length === 0)
      return this.runnable.mocha.bail;

    this.runnable.mocha.bail = Boolean(enabled);
  ...
  diff(x, y, msg) {
    if (msg == null) {
      const a = inspect.type(x);
      const b = inspect.type(y);
  ...
  show(value, msg, options) {
    if (typeof msg !== 'string') {
      options = msg;
      msg = inspect.type(value);
    }
