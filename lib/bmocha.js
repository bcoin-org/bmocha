...
class Mocha extends Runnable {
  constructor(options) {
    ...
    this.fullTrace = false; // noop
    this.inlineDiffs = false; // noop
  ...
  set(options) {
    ...
    if (options.inlineDiffs != null)
      this.inlineDiffs = Boolean(options.inlineDiffs);
  ...
  reject(error) {
    if (this.test) {
      ...
      // resolves" event.
      if (!this.test.fail) {
        this.test.skip = false;
        this.test.fail = true;
        this.test.error = toError(error);
        return;
      }
    }
...
class Suite extends Runnable {
  constructor(parent, title = '') {
    ...
    this.globals = parent.globals.slice();
    this.running = false;
    this.stats = new Stats();
    this.context = new Context(this);
...
class Executable extends Runnable {
  ...
  async exec() {
    return new Promise((resolve, reject) => {
      ...
      if (this.mocha.isAborting || this.skip) {
        resolve();
        return;
      }
  ...
  checkGlobals(snapshot) {
    ...
    for (const name of globals) {
      if (snapshot.indexOf(name) !== -1)
        continue;

      if (this.globals.indexOf(name) !== -1)
        continue;
    ...
    this.skip = false;
    this.fail = true;
    this.error = new Error(`global leaks detected: ${leaks.join(', ')}`);
  }
  ...
  async run() {
    ...
    if (this.mocha.forbidPending && this.skip) {
      this.skip = false;
      this.fail = true;
      this.error = new Error('Pending test forbidden');
    }
...
class Context {
  ...
  globals(arr) {
    if (Array.isArray(arr))
      this.runnable.globals.push(...arr);
    return this;
  }
  ...
  _inject(func) {
    ...
    func.globals = this.globals.bind(this);
...
class Base {
  ...
  error(error, depth) {
    ...
    if (this.diff && isShowable(error)) {
      const text = show(error, this.colors);
  ...
  epilogue(mocha) {
    ...
    for (let i = 0; i < mocha.results.length; i++) {
      ...
      for (let j = 0; j < path.length; j++) {
        ...
        const col = error.uncaught ? 'warning' : 'title';
...
class DocReporter extends Base {
  ...
  testEnd(test) {
    ...
    if (test.fail)  {
      ...
      const stack = escape(toStack(test.error));
...
class TapReporter extends Base {
  ...
  testEnd(test) {
    ...
    if (test.fail) {
      ...
      const stack = toStack(test.error);
...
exports.before = function before(desc, func) {
  if (typeof global.before !== 'function')
    throw new TypeError('"before" is not available.');
  return global.before(desc, func);
};

exports.after = function after(desc, func) {
  if (typeof global.after !== 'function')
    throw new TypeError('"after" is not available.');
  return global.after(desc, func);
};

exports.beforeEach = function beforeEach(desc, func) {
  if (typeof global.beforeEach !== 'function')
    throw new TypeError('"beforeEach" is not available.');
  return global.beforeEach(desc, func);
};

exports.afterEach = function afterEach(desc, func) {
  if (typeof global.afterEach !== 'function')
    throw new TypeError('"afterEach" is not available.');
  return global.afterEach(desc, func);
};

exports.describe = function describe(title, func) {
  if (typeof global.describe !== 'function')
    throw new TypeError('"describe" is not available.');
  return global.describe(title, func);
};

exports.describe.only = function only(title, func) {
  if (!global.describe || typeof global.describe.only !== 'function')
    throw new TypeError('"describe.only" is not available.');
  return global.describe.only(title, func);
};

exports.describe.skip = function skip(title, func) {
  if (!global.describe || typeof global.describe.skip !== 'function')
    throw new TypeError('"describe.skip" is not available.');
  return global.describe.skip(title, func);
};

exports.it = function it(title, func) {
  if (typeof global.it !== 'function')
    throw new TypeError('"it" is not available.');
  return global.it(title, func);
};

exports.it.only = function only(title, func) {
  if (!global.it || typeof global.it.only !== 'function')
    throw new TypeError('"it.only" is not available.');
  return global.it.only(title, func);
};

exports.it.skip = function skip(title, func) {
  if (!global.it || typeof global.it.skip !== 'function')
    throw new TypeError('"it.skip" is not available.');
  return global.it.skip(title, func);
};
