...
class ConsoleStream extends Stream {
  ...
  write(str) {
    ...
    // Drop all escape codes that chrome doesn't understand.
    if (this.isTTY)
      str = str.replace(/\x1b\[([\?\d;]*)([a-zA-Z])/g, dropCSI);
...
function dropCSI(str, args, code) {
  assert(typeof str === 'string');
  assert(typeof args === 'string');
