...
class Mocha extends Runnable {
  ...
  async run(funcs) {
    ...
    try {
      await this._globalTeardown(ctx);
    } catch (e) {
  ...
  async plugin(mod, file) {
    ...
    if (mod.mochaHooks != null)
      await this._pushHooks(mod.mochaHooks, file);
  ...
  async _pushHooks(hooks, file) {
    if (typeof hooks === 'function')
      hooks = await hooks();

    if (Array.isArray(hooks) || hooks === null || typeof hooks !== 'object')
      throw new TypeError(`Invalid global hook: ${file}`);
  ...
  _rootHooks(suite) {
    for (const func of this.rootHooks.beforeAll)
      suite.before(null, func, this._rootHooks);

    for (const func of this.rootHooks.afterAll)
      suite.after(null, func, this._rootHooks);

    for (const func of this.rootHooks.beforeEach)
      suite.beforeEach(null, func, this._rootHooks);

    for (const func of this.rootHooks.afterEach)
      suite.afterEach(null, func, this._rootHooks);
  }
}
