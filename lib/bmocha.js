...
class JSONReporter extends Reporter {
  ...
  begin(suite) {
    ...
    if (suite.name) {
      const name = JSON.stringify(suite.name);

      this.log(`      "name": ${name},`, depth);
    }
  ...
  test(test) {
    ...
    if (test.failed) {
      error = {
        name: String(test.error.name || ''),
        type: test.error.type || null,
        code: test.error.code || null,
        message: String(test.error.message).trim().split('\n'),
        stack: test.stack.trim().split('\n')
      };
    }
