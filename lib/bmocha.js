    ...
    return new Promise((resolve, reject) => {
      ...
      if (callbackable) {
        ...
        if (this.body.length > 0) {
          job.reject(new Error('Arrow function `ctx` usage is deprecated. '
                             + 'Please switch to using a suite\'s context.'));
          return;
        }
...
function injectDeprecation(cb) {
  const get = () => {
    throw stackError(get, 'Arrow function `cb` usage is deprecated. '
                        + 'Please switch to using a suite\'s context.');
  };

  for (const key of ['bail',
                     'enableTimeouts',
                     'only',
                     'retries',
                     'skip',
                     'slow',
                     'timeout']) {
    Object.defineProperty(cb, key, { enumerable: false, get });
  }
}
