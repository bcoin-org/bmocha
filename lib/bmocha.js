...
class Mocha extends Runnable {
  ...
  reject(error) {
    if (this.test) {
      ...
      if (!this.test.failed) {
        ...
        this.test.message = toMessage(this.test.error);
        this.test.stack = toStack(this.test.error);
        return;
      }
    }
  ...
  error(error) {
    ...
    this.stream.write('\n');
    this.stream.write('    An error occurred outside of the test suite:\n');
    this.stream.write('\n');
    this.stream.write(padding + message + '\n');

    if (stack.length > 0) {
      this.stream.write('\n');
      this.stream.write(indent(stack, 3) + '\n');
    }
...
class Base {
  ...
  epilogue(mocha) {
    ...
    for (let i = 0; i < mocha.errors.length; i++) {
      ...
      this.write('  '
        + color('warning', `${id}) Uncaught Error`)
        + '\n');
      ...
      if (diff.isDiffable(error)) {
        const text = diff.createDiff(error, this);
      ...
      if (stack.length > 0) {
        const stack_ = color('stack', stack);

        this.write('\n');
        this.write(indent(stack_, 3) + '\n');
      }
