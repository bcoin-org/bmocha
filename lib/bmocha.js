...
class Executable extends Runnable {
  constructor(parent, name, title, body) {
    ...
    if (/[\x00-\x1f\x7f]/.test(name + title))
      throw new Error('Invalid test or hook title.');
...
class MarkdownReporter extends Base {
  ...
  suiteStart(suite) {
    ...
    this.buffer += this.title(suite.title) + '\n';
  }
...
class ConsoleStream extends Stream {
  constructor(console, isTTY) {
    ...
    if (isTTY == null) {
      const ps = typeof process === 'object' && process;
      const tty = ps && ps.stdout && ps.stdout.isTTY;
      const {chrome} = global;

      if (tty || (chrome && chrome.app))
        isTTY = console === global.console;
    }
...
function hasContextArgument(func) {
  assert(typeof func === 'function');
  ...
  const ch = func.prototype === undefined ? '=>' : '{';
  ...
  return str.endsWith('ctx', i + 1);
}

function isCallbackable(func) {
  return typeof func === 'function'
      && func.length > 0
      && !hasContextArgument(func);
}
