...
class Reporter {
  constructor(stream = nullStream, options = Object.create(null)) {
    assert(stream && typeof stream.write === 'function');
...
global.setImmediate = (() => {
  ...
  if (global.postMessage && !global.importScripts) {
    const onmessage = global.onmessage;
  ...
  const prefix = `bmocha-${rand.toString(32)}:`;
  ...
  const execute = (handle) => {
    if (running) {
      setTimeout(execute, 1, handle);
      return;
    }

    const cb = tasks[handle];
  ...
  const onmessage = ({source, data}) => {
    if (source !== global || typeof data !== 'string')
      return;
    ...
    if (data.startsWith)
      ...
      match = data.indexOf(prefix) === 0;

    if (!match)
      return;

    const handle = data.substring(prefix.length);
