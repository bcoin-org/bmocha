...
class Suite extends Runnable {
  ...
  async run() {
    if (!this.mocha.failZero && this.total() === 0)
      return true;
  ...
  async exec() {
    if (this.mocha.failZero && this.total() === 0) {
      if (!this.root || this.suites.length === 0) {
        const test = new Test(this, 'empty suite', noop, null);
        test.setError(new Error('No tests encountered.'));
