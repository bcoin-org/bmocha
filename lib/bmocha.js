...
class Mocha extends Runnable {
  ...
  _before(func) {
    return this._suite().before(func);
  }

  _after(func) {
    return this._suite().after(func);
  }

  _beforeEach(func) {
    return this._suite().beforeEach(func);
  }

  _afterEach(func) {
    return this._suite().afterEach(func);
  }
...
class Context {
  ...
  before(func) {
    return this.runnable.mocha.before(func);
  }

  after(func) {
    return this.runnable.mocha.after(func);
  }

  beforeEach(func) {
    return this.runnable.mocha.beforeEach(func);
  }

  afterEach(func) {
    return this.runnable.mocha.afterEach(func);
  }
...
class Reporter {
  ...
  get columns() {
    if (typeof this.stream.columns === 'number')
      return (this.stream.columns * 0.67) | 0;
    return 75;
  }
