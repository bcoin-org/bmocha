...
class Server extends EventEmitter {
  ...
  async handle(req, res) {
    ...
    if (req.method === 'GET' && req.url.startsWith('/file?path=')) {
      const field = decodeURIComponent(req.url.substring(11));
      const path = join(cwd, normalize(field));

      let msg, len;

      try {
        msg = fs.readFileSync(path, 'base64');
