...
const extraPaths = [cwd, join(cwd, 'node_modules')];
...
class Server extends EventEmitter {
  ...
  async browserify(code, file) {
    ...
    try {
      bpkg = pathRequire('bpkg', globalPaths);
    } catch (e) {
  ...
  async _browserify(code, file) {
    ...
    try {
      browserify = pathRequire('browserify', globalPaths);
      builtins = pathRequire('browserify/lib/builtins', globalPaths);
    } catch (e) {
  ...
  async compileSuite() {
    ...
    for (const file of options.requires) {
      ...
      const path = pathResolve(file, extraPaths);
